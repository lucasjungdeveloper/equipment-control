...
import Axios from "axios"; // Importação do Axios para efetuar o registro de informação no banco (não compreendi muito bem, entendi o basico do basico)
import CardPrestador from "./CardPrestador"; // Importação do CardPrestador para mostrar na minha rota de Aparelhos x Prestadores a informação retornada do banco, relacionada ao prestador
import { FcSearch } from "react-icons/fc"; // Importação do incone de pesquisa, utilizado para pesquisar os prestadores
import { Prestador } from "./InterfacePrestador";
...
export function ConnectDeviceProvider() {
  ...
  async function getPrestador() {
    try {
      let { data, error } = await supabase.from("prestador").select("*");
