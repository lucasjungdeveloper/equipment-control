...
import CardDevice, { Props } from "./CardDevice";
import React, { PropsWithChildren, useEffect, useRef, useState } from "react";

import { Device } from "./InterfaceDevice";
import { FcSearch } from "react-icons/fc";
import { supabase } from "../supabase/supabaseClient";

export function DeviceControl(props: Props) {
  ...
  async function getDevice() {
    try {
      let { data, error } = await supabase.from("equipamento").select("*");

      if (error) throw error;
      if (data != null) {
        setListEquipment(data); //Aqui eu estou tentando setar o state listEquipment com o valor que vem do banco de dados
      }
    } catch (error: any) {
      alert(error.message);
    }
